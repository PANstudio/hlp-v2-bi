...
def db_connection_mutator(sqlalchemy_uri, database, username, security_manager, *args, **kwargs):
    target_db = getattr(g.user, "username", None)
    print(f"!!! Request to repoint SS to {target_db} DB")
    if target_db in DB_MAP:
        cfg = DB_MAP[target_db]
        sqlalchemy_uri = f"postgresql://{cfg['user']}:{cfg['password']}@{cfg['host']}/{cfg['dbname']}"
    print(f"!!! Repointing SS to {target_db} DB")
